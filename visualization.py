...
from src.loader import read_usedata, load_MR_dataset_images
...
def visualize_for_single(config, model, accelerator):
    model.eval()
    if config.trainer.choose_dataset == "GCN":
        from src.loader import get_GCM_transforms as get_transforms
        root = config.GCM_loader.root + '/' + 'ALL' + '/'
        choose = config.visualization.visual.GCM.choose_image
        use_config = config.GCM_loader
        write_path = config.visualization.visual.GCM.write_path
    elif config.trainer.choose_dataset == "GCNC":
        from src.loader import get_GCNC_transforms as get_transforms
        root = config.GCNC_loader.root  + "/" + "ALL" + "/"
        choose = config.visualization.visual.GCNC.choose_image
        use_config = config.GCNC_loader
        write_path = config.visualization.visual.GCM.write_path

    choose_image = root + '/' + choose
    ...
    models = os.listdir(choose_image)

    # for i in range(len(use_config.checkModels)):
    for i in range(len(models)):
        if "WI" in models[i]:
            check_model = models[i].replace("WI", "")
        elif models[i] == "CT1":
            check_model = "T1+C"
        elif models[i] == "T1+c":
            check_model = "T1+C"
        ...
        if check_model in use_config.checkModels:
            image_path = choose_image + "/" + models[i] + "/" + f"{choose}.nii.gz"
            label_path = (
                choose_image + "/" + models[i] + "/" + f"{choose}seg.nii.gz"
            )
    ...
    inference = monai.inferers.SlidingWindowInferer(
        roi_size=use_config.target_size,
        overlap=0.5,
    ...
    for i in range(len(models)):
        ...
        original_str = f"{save_path}/{choose}_inference.nii.gz"
...
def visualize_for_all(config, image_list, model, accelerator):
    ...
    if config.trainer.choose_dataset == "GCN":
        from src.loader import get_GCM_transforms as get_transforms
        root = config.GCM_loader.root + "/" + "ALL" + "/"
        choose = config.visual.GCM.choose_image
        use_config = config.GCM_loader
    elif config.trainer.choose_dataset == "GCNC":
        from src.loader import get_GCNC_transforms as get_transforms

        root = config.GCNC_loader.root + "/" + "ALL" + "/"
        choose = config.visual.GCNC.choose_image
        use_config = config.GCNC_loader
...
if __name__ == '__main__':
    ...
    #     "model_store" + "/" + f"{finetune.checkpoint}" + "/best/model.pth",
    #     model,
    #     accelerator,
    # )
