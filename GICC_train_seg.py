...
from src.loader import get_dataloader_GICC as get_dataloader
...
def train_one_epoch(
    ...
    for metric_name in metrics:
        ...
        for i in range(len(config.GICC_loader.checkModels)):
            metric_dice[f"Train/ {config.GICC_loader.checkModels[i]}"] = float(
                batch_acc[i]
            )
...
def val_one_epoch(
    ...
    for metric_name in metrics:
        ...
        for i in range(len(config.GICC_loader.checkModels)):
            metric_dice[f"{flag}/ {config.GICC_loader.checkModels[i]}"] = float(
                batch_acc[i]
            )
        metric.update(metric_dice)
...
if __name__ == "__main__":
    ...
    if config.finetune.GICC.checkpoint != "None":
        checkpoint_name = config.finetune.GICC.checkpoint
    else:
        checkpoint_name = (
    ...
    inference = monai.inferers.SlidingWindowInferer(
        roi_size=config.GICC_loader.target_size,
        overlap=0.5,
